var https = require('https')
  ...
  , fs = require('fs')
  , caFile = fs.readFileSync(__dirname + '/shared/cert.pem').toString();
  ...
  AuthorizeRequest.prototype.send = function(action, xml, options) {
    ...
    .on('response', function(res) {
      if (!!res.connection.authorizationError) {
        return callback(res.connection.authorizationError, null);
      }
      ...
      res.on('data', function(data) {
        ...
        if (resultCode != "OK" && resultCode != "SUCCESS") {
          return callback(values.messages.message)
        }
